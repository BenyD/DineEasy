-- Update create_order_with_items function to set orders to 'preparing' status
-- This ensures all orders (cash and card) start from 'preparing' status when created
...
BEGIN
  -- Start transaction
  BEGIN
    -- Create the order with 'preparing' status instead of 'pending'
    INSERT INTO orders (
      id,
      restaurant_id,
      table_id,
