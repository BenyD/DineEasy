-- Update upsert_subscription function to support metadata for trial upgrade tracking
...
begin
  ...
  -- Log the attempt
  raise log 'Attempting to upsert subscription: % for restaurant: % with timestamps: % and metadata: %', 
    p_stripe_subscription_id, p_restaurant_id, v_has_timestamps, p_metadata;
  ...
  -- Log success
  raise log 'Successfully upserted subscription: % with metadata: %', p_stripe_subscription_id, p_metadata;
...
grant execute on function upsert_subscription(text, uuid, text, text, text, text, text, timestamp with time zone, timestamp with time zone, timestamp with time zone, timestamp with time zone, timestamp with time zone, timestamp with time zone, jsonb) to authenticated;

-- Log completion
do $$
begin
  raise log 'upsert_subscription function updated with metadata support';
end $$; 